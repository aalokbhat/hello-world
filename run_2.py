...
def main(pickleloc):
    print(pickleloc)
# def main(pickleloc="data/Wiki-Vote-Analysis")
    ...
    def SampleRandomPairApproximationDistanceStats(g):
     ...
     pickleloc = '/home/aalok/Documents/amdm_ass/a/data/Wiki-Vote.txt'
    pciklefnames = glob(pickleloc + "/" + pickleloc.split("/")[-1].split(".")[0].split("-Analysis")[0] + "*.pickle")
    print(pciklefnames)
    #with open(pciklefnames[0], 'rb') as handle:
    with open('data/Wiki-Vote-Analysis/Wiki-Vote-lscc-graph.pickle', 'rb') as handle:
        lscc = pickle.load(handle)['glscc']

    #with open(pciklefnames[1], 'rb') as handle:
    with open('data/Wiki-Vote-Analysis/Wiki-Vote-lwcc-graph.pickle', 'rb') as handle:
